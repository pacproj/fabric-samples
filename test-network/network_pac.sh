...
function createOrgs() {
  ...
  ./organizations/pac-ccp-generate.sh
}
...
function createChannels() {
  ...
  scripts/createPACChannels.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE
}


## Call the script to deploy a chaincode to the channel
function deployCC() {
  scripts/deployPAC-CC.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE $ORGANISATION
...
COMPOSE_FILE_BASE=docker/docker-compose-test-pac-net.yaml
# docker-compose.yaml file if you are using couchdb
COMPOSE_FILE_COUCH=docker/docker-compose-pac-couch.yaml
# certificate authorities compose file
COMPOSE_FILE_CA=docker/docker-compose-pac-ca.yaml
